...
static inline size_t min_size(size_t s1, size_t s2) {
    return (s1 < s2) ? s1 : s2;
}
...
void spi_read(SPI_Address_t address, size_t size, uint8_t buf[]) {
    ...
    switch (address) {
        ...
        case ADDRESS_CONTROLLER_COLOR: {
            if (size >= 3) {
                memcpy(buf, controller_color, sizeof(controller_color));
            }
            if (size >= 6) {
                memcpy(buf + 3, button_color, sizeof(button_color));
            }
            if (size >= 9) {
                memcpy(buf + 6, left_grip_color, sizeof(left_grip_color));
            }
            if (size >= 12) {
                memcpy(buf + 9, right_grip_color, sizeof(right_grip_color));
            }
            break;
        }
        case ADDRESS_FACTORY_PARAMETERS_1: {
            ...
            memcpy(buf, factory_parameters_1, min_size(size, sizeof(factory_parameters_1)));
            break;
        }
        case ADDRESS_FACTORY_PARAMETERS_2: {
            ...
            memcpy(buf, factory_parameters_2, min_size(size, sizeof(factory_parameters_2)));
            break;
        }
        case ADDRESS_FACTORY_CALIBRATION_1: {
            ...
            memcpy(buf, factory_calibration_1, min_size(size, sizeof(factory_calibration_1)));
            break;
        }
        case ADDRESS_FACTORY_CALIBRATION_2: {
            uint8_t factory_calibration_2[] = {0xba, 0x15, 0x62, 0x11, 0xb8, 0x7f, 0x29, 0x06, 0x5b, 0xff, 0xe7, 0x7e,
                                               ...
                                               0xff};
            memcpy(buf, factory_calibration_2, min_size(size, sizeof(factory_calibration_2)));
            break;
        }
        ...
        case ADDRESS_IMU_CALIBRATION: {
            ...
            memcpy(buf, imu_calibration, min_size(size, sizeof(imu_calibration)));
            break;
        }
    }
}
