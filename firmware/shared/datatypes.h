...
// UART commands (for relay firmware)
typedef enum {
    COMMAND_NOP         = 0,
    COMMAND_SYNC_1      = 0x33,
    COMMAND_SYNC_2      = 0xCC,
...
// UART responses (for relay firmware)
typedef enum {
    RESP_USB_ACK        = 0x90,
...
// Serial input packet structure (for relay firmware)
typedef struct {
    uint8_t input[8];
    uint8_t crc8_ccitt;
...
// Note: Bit order in each byte is inverted
typedef struct {
